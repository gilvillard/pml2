...
{
    printf("Usage: %s OR %s [nbits] [rdim] [degA] [order] [sigma] [Bcdim] [degB] \n--\n", argv[0], argv[0]);
    ...
    else if (argc == 8)
    {
        ...
        slong degA = atoi(argv[3]);
        slong order = atoi(argv[4]);
        slong sigma = atoi(argv[5]);
        slong Bcdim  = atoi(argv[6]);
        slong degB = atoi(argv[7]);


        slong prime = n_randprime(state, nbits, 1);
        printf("Launching  with\n\tprime = %ld,\n\trdim = %ldx%ld,\
            \n\tdegA = %ld,\n\torder = %ld,\n\tsigma = %ld,\
            \n\tBcdim = %ldx%ld,\n\tdegB = %ld ...\n",prime, rdim, rdim, degA, order, sigma, rdim, Bcdim, degB);

        nmod_poly_mat_t A;
        nmod_poly_mat_init(A, rdim, rdim, prime);
        nmod_poly_mat_rand_origin(A, state, degA+1);
        ...
        nmod_poly_mat_rand(B, state, degB+1);
